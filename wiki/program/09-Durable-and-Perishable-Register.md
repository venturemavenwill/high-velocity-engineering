...
| The playbook's recipes: its step-by-step procedures, each bound to a named tool at a named version, indistinguishable in the navigation from the practice pages beside them | PE strand, wherever a playbook page is assigned | A standard mixes durable practice with tool-bound procedure and does not label which is which — so a reader must classify a page before relying on it, and the classification is not supplied |

**The recipe row is the only entry in this table whose perishable instance is a genre rather than a product**, and it is here rather than in the general perishable table for the reason the section opens with: it decays on a vendor's schedule and not on the document's. A recipe page may be cited for the existence of an approach and for nothing else. **No day may teach one as the standard**, and a day that assigns a playbook section assigns it with the classification exercise attached — which is [S036](/wiki/seminars/S036.md)'s reading pass.
...
Carried from [research/06-microsoft-platform/](/research/06-microsoft-platform/collected-materials.md), and binding on every page and every session:
...
- **Any productivity, quality, defect-rate, safety, correctness or learning outcome attributed to an agent configuration corpus**, by its authors or by anyone else. At the pinned revision the corpus recorded at [research/06-microsoft-platform/hve-core-repository.md](/research/06-microsoft-platform/hve-core-repository.md) contained evaluation machinery and **no committed result corpus**, while its own prose used the vocabulary of demonstrated benefit. **The machinery is real and it is not a result**, and the gap between the two is taught rather than tidied away.
- **Any outcome attributed to a delivery standard or an engineering playbook.** Neither [research/05-fde-craft/microsoft-fde-delivery-practice.md](/research/05-fde-craft/microsoft-fde-delivery-practice.md) nor [research/05-fde-craft/microsoft-ise-engineering-playbook.md](/research/05-fde-craft/microsoft-ise-engineering-playbook.md) reports an evaluation, a comparison or a measurement; the phase durations are plans and the AI-generation shares are assertions. **That following a standard produces better engagements is not a finding**, and no session may assert it.
- **That the cost of code review is negative.** The playbook's own code-review page asserts it with no evidence. **Amended 2026-07-30: it is one of four.** A reading of that source in full found the same failure in four sections — this one, an assertion about a team's delivery rate under a named collaboration practice, a multiple for what a late-found defect costs attributed through a link that cannot be followed, and vendor efficiency figures banded too widely to be contradicted. **None of the four figures is reproduced anywhere in this repository**, and the entry is kept in the singular above so the correction stays legible. **A single instance is a slip, and four is a property of the document** — that it does not distinguish a practice it reasoned about from a number it repeated, while writing both in the same voice.
...
- [Programme Map](/wiki/program/01-Programme-Map.md) — the nine-stage arc this register applies to
- [Whitepaper Standard](/wiki/program/05-Whitepaper-Standard.md) — the four evidence classes
- [Assessment Architecture](/wiki/program/03-Assessment-Architecture.md) — where the rule that perishable content is never assessed is enforced
- [Module Map](/wiki/program/07-Module-Map.md)
- [Microsoft platform sources](/research/06-microsoft-platform/collected-materials.md) — the folder this register's platform layer is drawn from
- [Agent configuration corpus, read at a pinned commit](/research/06-microsoft-platform/hve-core-repository.md)
- [Namespaces](/concepts/namespaces.md) — decay rates by namespace, of which this register is the platform-layer instance
