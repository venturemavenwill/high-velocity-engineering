...
Teams and individuals learn from error only in environments where reporting error is not punished. Error management training — which explicitly frames errors as informative and encourages exploration — outperforms error-avoidant training on transfer tasks, with the advantage concentrated in tasks unlike those practised. Psychological safety is a reliable predictor of learning behaviour and is established primarily by leader behaviour, including visible fallibility.

**Design consequence.** Phase 6 of the seminar day is an **error clinic** in which errors are surfaced deliberately. Staff model fallibility explicitly. Incident review on the substrate is blameless by protocol and the protocol is published. Verified readings: [research/04-professional-formation/keith-frese-error-management-training.md](research/04-professional-formation/keith-frese-error-management-training.md), [research/04-professional-formation/edmondson-psychological-safety.md](research/04-professional-formation/edmondson-psychological-safety.md), [research/04-professional-formation/frazier-psychological-safety-meta-analysis.md](research/04-professional-formation/frazier-psychological-safety-meta-analysis.md).

**Scope conditions and limits.**
- Psychological safety is necessary and not sufficient; it predicts learning behaviour, not learning outcome, and its effect on performance is mediated.
- Error management framing can be misread as tolerance of carelessness. The programme distinguishes explicitly between *error in exploration*, which is encouraged, and *error in verification*, which is not.
...
**Design consequence.** Phases 3 and 4 are ordered **problem-first, instruction-second**, deliberately and against convention. Phase 3 uses contrasting cases to make the relevant distinctions noticeable without naming them. Verified reading: [research/04-professional-formation/sinha-kapur-productive-failure.md](research/04-professional-formation/sinha-kapur-productive-failure.md).

**Scope conditions and limits.**
- The consolidation phase is not optional. Productive failure without a well-designed instructional follow-up is simply failure.
- Effects are strongest for conceptual understanding and transfer, and are frequently null or negative for procedural fluency.
...
Beliefs about one's capability to execute a task determine effort, persistence and choice of challenge, and are formed principally from mastery experience, then vicarious experience, social persuasion and interpretation of physiological state. Mastery experience dominates.

**Design consequence.** Early task classes are engineered so that genuine, non-trivial success is reachable — not by lowering standards but by controlling complexity class. The credit structure recognises demonstrated capability rather than accumulated points. Vicarious experience is supplied by near-peer demonstration in phase 7, with second-year students visible to first-year cohorts on the shared substrate. Verified reading: [research/04-professional-formation/bandura-self-efficacy.md](research/04-professional-formation/bandura-self-efficacy.md).

**Scope conditions and limits.**
- Self-efficacy that outruns capability degrades calibration (§11). The two must be balanced rather than maximised independently.
...
Full design in [Assessment Architecture](wiki/program/03-Assessment-Architecture.md).
