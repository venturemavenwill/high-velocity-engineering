...
- **An internal Microsoft HVE workshop enablement hub** — a curated site of 40 markdown documents (4,233 lines) covering a four-pillar model introduction, a workshop delivery lifecycle, facilitator guides, a customer discovery questionnaire, a customer-facing FAQ, two maturity matrices and an examples folder. **Every page declares an internal audience in its own front-matter and one is marked not-for-customer-sharing; not publicly reachable from the copy read and not citable externally.** Read in full on 2026-08-06. Recorded only as a **public paraphrase**: no link, no rendered title, no repository name, no individual, no customer. Note: [microsoft-hve-enablement-hub.md](/research/05-fde-craft/microsoft-hve-enablement-hub.md).
  - **Its one populated example could not be read, and that is recorded as a finding rather than a gap in the reading.** The document is present twice: a markdown version that terminates at its *outcomes* heading with nothing beneath it, and a binary version that is rights-protected and encrypted, carries an information-protection label, and is misnamed with the modern document extension while actually being the legacy compound-file format. **Its cleartext metadata exposes an author name and a tenant identifier; neither is reproduced.**
  - **This hub substantially overlaps the accelerated-delivery corpus above and is retained as a separate note deliberately**, because a second independently arranged presentation of the same practice is the only means available here of testing whether that note's findings were properties of the material or artefacts of how it was read.
- **A vendor partner summit on frontier transformation and hypervelocity engineering** — the complete English transcript of two tracks, roughly 2 hours 55 minutes of speech across 3,613 caption cues, convened to launch a partner specialization. **Gated behind partner enrolment; not publicly reachable from the copy read and not citable externally.** Read in full on 2026-08-06. Recorded only as a **public paraphrase**: no link, no speaker, no customer organisation, no programme address, no certification identifier. Note: [microsoft-hve-partner-summit.md](/research/05-fde-craft/microsoft-hve-partner-summit.md).
  - **Two provenance caveats travel with it.** It is derived from machine captions and says so; product names and acronyms are mangled throughout, and **no claim in the note rests on a figure that appeared only as a spoken number.** And the slides the speakers continually refer to are not in evidence, so the transcript is one channel of a two-channel artefact.
  - **It is the most magnitude-dense source in this register and the least warranted.** Roughly thirty quantitative assertions across both tracks, none carrying an instrument, baseline, period, denominator or comparison. **One is an analyst market projection, which is a claim class the [standing prohibition list](/wiki/program/09-Durable-and-Perishable-Register.md) forbids outright**; it is recorded as seen and refused. Several are customer-reported improvements relayed by the supplier who produced them, one explicitly framed as the customer's own claim.
  - **Its value is a failure narrative and a mechanism claim, not its figures.** It supplies the worked account of a delivery that met every criterion and produced nothing — a composite by its speaker's own statement — and the claim that forbidding implementation during a research phase changes what a model optimises for. It is also the **third independent confirmation** that this organisation names external measurement frameworks and reports no measurement from any of them.
- **What was deliberately excluded from these four notes, recorded so a later reader knows the omission was a decision rather than an absence:** a roster of named individuals holding a security role; **two named customer case studies plus four further named customer deployments described in the summit**; named customer leadership roles; **eight named vendor speakers and their titles**; internal tenant, environment and application identifiers; an individual author name recovered from binary metadata; internal escalation routing, mailboxes and ticket queues; internal role-group and audience labels; **a partner specialization name, its badge, its certification identifiers and its enrolment addresses**; platform-specific injection detail and worked exploit examples; and every figure asserted without a source — of which the delivery corpus contains roughly thirty, the agent pipeline roughly fifteen, the enablement hub a set embedded in questionnaire specimen answers, and the summit roughly thirty more. **All four notes state the existence and subject of each figure and reproduce none of them.**
...
| Source | Identifier | Access | Read |
|---|---|---|---|
| Meppiel, *The Agentic SDLC Handbook*, v0.11.0 (June 2026) | `https://danielmeppiel.github.io/agentic-sdlc-handbook/` | open web, free | **preface, thesis chapter and primary case study read in full; Parts II–III, three case studies and both appendices not read** |
...
### CAIRA reference architectures — read 2026-08-06
Note: [caira-reference-architectures.md](/research/06-microsoft-platform/caira-reference-architectures.md). **A public Microsoft repository, citable normally.** All 133 files read in full: 17 markdown totalling 317 lines, plus TypeScript, C#, Terraform, shell and workflow definitions. Its substance is code rather than prose, deliberately — the bundled skill instructs agents not to stop at the README.

**Two limitations belong with it.** It was read as a **downloaded snapshot of the default branch with no recoverable commit identifier**, so unlike the agent-configuration corpus it cannot be pinned to a revision and a later reader cannot reconstruct exactly what was read; the declared skill version is a package version, not a revision. And **it reports no outcome of any kind** — no measurement, no comparison against building from scratch, no adoption data. Its three-sentence transparency note names no limitation, failure mode or human-oversight requirement.

| Source | URL | Read |
|---|---|---|
| CAIRA — Composable AI Reference Architectures, skill version 0.4.3 | `https://github.com/microsoft/CAIRA` | **all 133 files read in full — public repository, snapshot without a commit identifier, so unpinnable; no outcome, comparison or adoption data reported** |

### Adoption hubs and the low-code guidance layer — read 2026-08-06
Note: [microsoft-adoption-hubs-copilot-studio-power-platform.md](/research/06-microsoft-platform/microsoft-adoption-hubs-copilot-studio-power-platform.md). **Public marketing hubs plus roughly thirty Microsoft Learn guidance pages, citable normally.** This is the source of the fusion personas at [S005](/wiki/seminars/S005.md), the governance control-latency mechanism at [S057](/wiki/seminars/S057.md), and the maker as an affected non-buyer at [S007](/wiki/seminars/S007.md).

**Two access findings belong with it.** The **Power Platform adoption hub returned content in the morning of 2026-08-06 and a 404 in the afternoon of the same day** — its reading is retained and is now unverifiable by any later reader. The **Copilot Studio hub silently redirects** to an `/ai-agents/` path and has been restructured into a three-product page, so the URL a citation names is not the page a reader reaches.

**The hubs themselves are routing tables** carrying no method, measurement or outcome; the substance is one hop away, and the first version of this note stopped at the hub and drew a conclusion from the wrong layer. **The guidance layer beneath is materially better** — first-party documentation stating mechanisms and publishing comparisons — but still **reports no outcome data of any kind**, so no effect size may be asserted from any of it.

**One prohibited artefact was encountered and refused.** The staged agentic-adoption maturity model is linked from both the hub and the guidance. [09-Durable-and-Perishable-Register.md](/wiki/program/09-Durable-and-Perishable-Register.md) forbids asserting it and nothing from it is carried into the wiki as a claim; it is recorded as **seen and refused** rather than quietly omitted. A ten-minute readiness self-assessment is likewise offered with **no published scoring, calibration or validation**.

**The library contradicts itself**, which is a finding rather than a complaint: the CoE Starter Kit is marked **no longer actively maintained** on its own page while at least four other pages recommend it in the present tense and one advises updating it monthly. **This is the second specimen of the per-page-currency pattern**, the first being the Microsoft IQ family.

| Source | URL | Read |
|---|---|---|
| Copilot Studio adoption hub | `https://adoption.microsoft.com/en-us/ai-agents/copilot-studio/` | **read in full 2026-08-06 — routing table; silently redirected from the cited path; restructured to three products; links a staged maturity model, seen and refused; self-assessment uncalibrated; no outcome data** |
| Power Platform adoption hub | `https://adoption.microsoft.com/en-us/power-platform/` | **read in full 2026-08-06 morning; the same URL returned 404 the same afternoon — reading retained but now unverifiable** |
| Fusion development in Power Platform | `https://learn.microsoft.com/power-platform/developer/fusion-development` | **read in full — three personas and the stated division of labour; term attributed to a 2019 analyst survey not read here** |
| Collaborating and co-developing apps using Power Apps | `https://learn.microsoft.com/power-apps/guidance/co-develop/overview`, `/collaboration` | **read in full — four-role minimum team; siloing and shadow IT named as the failure mode; no outcome data** |
| Implement reactive governance controls | `https://learn.microsoft.com/power-platform/guidance/adoption/reactive-governance` | **fetched and read in full — the public-API-versus-in-product control-latency mechanism, the twenty-recipient worked example, and the per-feature deactivation table; no effectiveness or false-positive measurement published** |
| Tenant environment strategy at scale | `https://learn.microsoft.com/power-platform/guidance/adoption/environment-strategy` | **read in full — graduated solution-checker enforcement at none/warn/block with per-environment recommendations carrying no stated justification** |
| Manage and govern the default environment | `https://learn.microsoft.com/power-platform/guidance/adoption/manage-default-environment` | **read in full — universal employee access and default environment routing** |
| CoE Starter Kit overview and modules | `https://learn.microsoft.com/power-platform/guidance/coe/overview`, `/starter-kit-explained` | **read in full — carries the no-longer-actively-maintained notice that four other pages contradict** |
| Power Platform Well-Architected — operational excellence | `https://learn.microsoft.com/power-platform/well-architected/operational-excellence/principles`, `/fusion-culture` | **read in full — fusion development as a named design principle under OE:01** |
| Copy an agent to Copilot Studio | `https://learn.microsoft.com/microsoft-365/copilot/extensibility/copy-agent-to-copilot-studio` | **read in full — the third build tier and what promotion confers; no usage, cost or success data** |
| Deutsche Bahn citizen-developer case study | `https://learn.microsoft.com/power-platform/guidance/case-studies/db-empowers-citizen-devs` | **read in full — a named promotion gate operated by a party other than the builder; vendor-selected success with no denominator, comparison or failure case** |
| Agentic AI adoption maturity model | `https://learn.microsoft.com/agents/adoption-maturity-model/` | **read — prohibited; nothing carried into the wiki as a claim; recorded as seen and refused** |
