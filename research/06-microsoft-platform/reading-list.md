...
## What does a vendor's own agent configuration look like when it is published?
- **`microsoft/hve-core`** — `https://github.com/microsoft/hve-core`. Open public repository, **cloned and machine-read in full at commit `cf29fb457b0fe62745bb71592ec9394b834957f4`, package version 3.3.101, dated 2026-07-29.** Answers: the four artifact kinds and why they are not four names for one thing; the separation of authoring source, packaging manifest and generated output; the RPI phase model with gates and return paths; durable on-disk state under stable identifiers; review decomposed into orientation, lens, severity and depth tier; the untrusted-content boundary; human approval as a stop condition rather than a checkbox; and the advisory-versus-authoritative gate distinction. Note: [hve-core-repository.md](/research/06-microsoft-platform/hve-core-repository.md).
- **The gap is the finding.** The repository contains evaluation machinery and **no committed result corpus**, while its prose uses benefit vocabulary — "ship faster", "prevents", "proven". Nothing in it establishes a productivity, quality, safety or learning outcome, and the note records the search that established that rather than asserting it.
- Not fetched: the upstream host clients that load the artifacts, whose behaviour every claim in the repository depends on and which are versioned independently of it.
