...
### HVE Core, the agent configuration repository
Full note: [hve-core-repository.md](/research/06-microsoft-platform/hve-core-repository.md)
- Read at commit `cf29fb457b0fe62745bb71592ec9394b834957f4`, package version 3.3.101, 2026-07-29. **An unpinned citation to a repository is a citation to whatever it says today**, so the pin is part of the claim.
- Four artifact kinds with distinct application models: **instructions** (applied automatically to matching work), **prompts** (invoked by a person), **agents** (declared roles and orchestration), **skills** (reusable procedures with their own assets).
- **Authoring source, packaging manifest and generated output are separated**, and the generated tree is declared non-canonical. Maturity and retirement are expressed by collection membership, not inferred from file age.
- **RPI** — Research, Plan, Implement, Review, Follow-up — as gated phases with stable task identity, blocker rules, review outcomes and explicit return paths to earlier phases.
- **Durable on-disk state is declared authoritative over session memory**, under stable identifiers, which buys provenance and restartability and buys nothing about correctness.
- **Execution completion and acceptance outcome are separated.** A phase that ran is not a phase that was accepted.
- Review is decomposed into factual orientation, fixed lenses, a normalised severity taxonomy and risk-scaled depth tiers, so two reviewers produce comparable findings.
- **Untrusted content is data, not authority.** Fetched pages, documents, repositories, MCP output and issue bodies may not issue instructions; embedded instructions are reported rather than obeyed. This is a declaration whose effect depends on the host and the model.
- **An agent may not check a human-review box**, and a backlog workflow stops when a required human review is absent. The transparency note states the project ships no model and no safety classifier and that host behaviour varies.
- **A gate may be advisory or authoritative depending on the tier it runs in**, and regex-graded behavioural checks are documented by the repository itself as testing output shape rather than correctness.
- **The machinery exists and the results do not.** No committed study, baseline, dataset or result corpus was found at the pinned revision; result storage is an empty placeholder. The prose nevertheless uses "ship faster", "prevents" and "proven".
- **Must not be taught as fact:** any productivity, quality, safety, correctness, risk-reduction or learning outcome attributed to this corpus, by it or by anyone else.
...
- **Every source states a latency or a sampling bound, and none states an accuracy.** Assessment cadence, sync lag, policy propagation and result expiry are all documented precisely. Detection rates, false-positive rates and judge agreement are documented nowhere. The asymmetry is consistent enough across seven independent sources to be a property of vendor material as a genre.
- **One source declares its own silence, and that is the exception worth teaching.** `microsoft/hve-core` states in its own documentation that its behavioural graders test output shape rather than correctness, that some gates are advisory, and that host behaviour varies. **It is still silent on accuracy** — no outcome is measured — but the silence is announced rather than left to be discovered. The genre pattern holds; what varies is whether the reader has to find the gap themselves, and a source that names its own limits is the better teaching object precisely because its candour cannot be mistaken for evidence.
- **A control system is not an evidence system.** The most complete engineering process in this folder is also the one with no committed results, and the two facts sit in the same repository without contradiction. Nothing about the quality of a control licenses a claim about its effect.
...
- **The 36% retrieval claim is verifiable after all, and worse than it looked. Amended 2026-07-31.** This entry previously read that the claim *remains unverified and probably unverifiable*, with no benchmark, metric definition, baseline system or interval anywhere. A methodology has since been located in first-party documentation and is recorded at [microsoft-iq-intelligence-layer.md](/research/06-microsoft-platform/microsoft-iq-intelligence-layer.md): named authors, six datasets, per-dataset results. **The prohibition is unchanged and its grounds are now firmer.** The experiment compared *medium* against *minimal* retrieval reasoning effort — the product against itself, turned down — and the vendor's FAQ restates that result as an improvement over traditional single-shot RAG, a comparison nobody ran. **A claim with a real methodology behind it that measures something other than what it says is more dangerous than one with no methodology at all**, because it survives the first check.
...
- **`microsoft/hve-core` is the only source here read at a pinned revision, and it is the only one where that was possible.** Learn pages carry no version; a repository does. Where a source can be pinned it must be, and the asymmetry is a property of the source type rather than of the care taken.
- **Open question:** whether the absence of a result corpus in `microsoft/hve-core` is a stage of the project or a property of it. Absence at one commit is not absence in the project, and the note must be re-run against a later revision before any stronger statement is made in either direction.
- **Open question:** whether the untrusted-content boundary holds on the hosts a course would actually use. The instruction is declarative and its enforcement is the host's; nobody has tested it here.
...
All external sources in this folder are open: Microsoft Learn documentation retrieved via the Learn documentation tools, and one open GitHub repository read at a pinned commit. Full URLs are recorded in [reading-list.md](/research/06-microsoft-platform/reading-list.md) and in each individual note's citation section. Two of the pages consulted are explicitly labelled classic or superseded, and their replacements have not yet been read. **The repository source is citable only with its commit and package version attached**, and a citation without them is not a citation. The internal course document `sources/vendor-courses/03-Accelerate-Agentic-AI.md` is **not a public source** and must not be cited externally; anything taught from it must first be re-grounded in public documentation.