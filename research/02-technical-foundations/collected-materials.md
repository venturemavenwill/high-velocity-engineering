...
### Crowe, Hapke, Caveness and Zhu, Machine Learning Production Systems (O'Reilly, first edition October 2024, ISBN 978-1-098-15601-5)
- A practitioner survey of everything in a production machine learning system that is not the model, organised around the claim that a model is usually about five per cent of the code required to put an application into production. It is the only source in this folder that treats the lifecycle surrounding a model rather than a mechanism inside one.
- Verified specifics: the research-versus-production contrast across data, competing priorities and lifetime; drift and skew as different comparisons, drift splitting into data drift and concept drift and skew into schema, feature and distribution skew; training–serving skew and the structural remedy of exporting preprocessing as a transform graph attached to the served model; the instance-level versus full-pass distinction that makes that remedy necessary; slicing as mandatory because aggregate metrics conceal subgroup failure, with slice choice a domain-knowledge question; entanglement and configuration as the two sensitivities ordinary software tests do not catch; the optimising-metric and gating-metric discipline for adding complexity; MLOps levels 0 to 2 with level 2 conceded as speculative; and the statement that equal fairness metrics across groups do not establish that a system is fair.
- Copyrighted; a locally held EPUB was read in full on 2026-07-30 and the note is an original synthesis quoting short attributed fragments only.
- Three of the four authors work at Google and the default toolchain throughout is TensorFlow Extended, Vertex and the surrounding Google stack. Read as a description of how one large organisation operates production ML it is strong evidence; read as a comparative evaluation of practices it is not evidence at all.
- Every magnitude in the book — the quantisation, pruning, distillation, routing and hardware figures — is a property of a particular system, and the authors' own vendor and first-party benchmarks are the weakest evidence in it. Direction and mechanism transfer; the numbers do not.
...
- The Crowe et al. title page carries an unremoved production-template line reading "Second Edition". The copyright page says October 2024, first edition, and is authoritative; a reader who saw only the title page would cite the wrong edition. The book must also not be confused with Kaestner and Kang, *Machine Learning in Production* (MIT Press, open access), which this repository lists separately in folder 07 and has not read in full. Different authors, different publisher, different book, and citing one for the other would misattribute both the claim and its warrant.
